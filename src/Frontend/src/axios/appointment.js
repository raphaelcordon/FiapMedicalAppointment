...
export const ScheduleAppointment = async (data) => {
  try {
    const config = getAxiosConfig();
    const res = await axios.post(`/Appointment/schedule`, data, config); // Wrap data in scheduleDto
    return res.data;
  } catch (error) {
...
export const UpdateAppointmentStatus = async (appointmentId, updateData) => {
  try {
    const config = getAxiosConfig();
    const res = await axios.post(`/Appointment/update/${appointmentId}`, updateData, config);
    return res.data;
  } catch (error) {
